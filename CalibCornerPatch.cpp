...
CalibCornerPatch::CalibCornerPatch(int nSideSize)
{
  ...
  //if(mimSharedSourceTemplate.cols == 0)  {
  // construct the putative corner and store in mimSharedSourceTemplate   
  MakeSharedTemplate();
  // }
}
...
void CalibCornerPatch::MakeTemplateWithCurrentParams()
{
  ...
  cv::Mat_<float> imTwiceToBlur(imToBlur.rows * 2, imToBlur.cols * 2);
  ...
  int nOffsetCol, nOffsetRow;
  {
    ...
    // halfsampling with cv::resize(). interpolation is bilinear by default.
  ...
  for(int dof=0; dof<2; dof++) {
	...
	for (c = 0; c < mimTemplate.cols; c++)
	  mimAngleJacs(r, c)[dof] = ( imToBlur(r + nOffsetRow, c + nOffsetCol) - mimTemplate(r, c) ) / 0.01;
  ...
  for (r = 0; r < mimGradients.rows; r++)
    ...
    {
      mimGradients(r, c)[0] = 0.5 * ( imToBlur(r + nOffsetRow, c + nOffsetCol + 1) - 
				      imToBlur(r + nOffsetRow, c + nOffsetCol - 1) );
      mimGradients(r, c)[1] = 0.5 * ( imToBlur(r + nOffsetRow + 1, c + nOffsetCol) - 
				      imToBlur(r + nOffsetRow - 1, c + nOffsetCol ) );
    }
...
{ 
  ...
  cv::Mat_<double> m6JTJ = cv::Mat_<double>::zeros(6,6); // this is the J'*J (can be though of as information matrix)
  cv::Mat_<double> v6JTD = cv::Mat_<double>(6, 1);       // J' * error (think information vector)
  ...
  for (r = 0; r < mimTemplate.rows; r++)
    for (c = 0; c < mimTemplate.cols; c++)
    {
      ...
      double dDiff = imInterp[v2Pos_Image][0] - (mParams.dGain * mimTemplate(r, c) + mParams.dMean);
      ...
      cv::Mat_<double> v6Jac(6,1);
      // First two derivatives are in terms of position
      v6Jac(0, 0) = -1.0 * mParams.dGain * mimGradients(r, c)[0]; 
      v6Jac(1, 0) = -1.0 * mParams.dGain * mimGradients(r, c)[1]; 
      // The next two derivatives are in terms of the angles
      v6Jac(2, 0) = mimAngleJacs(r, c)[0] * mParams.dGain; 
      v6Jac(3, 0) = mimAngleJacs(r, c)[1] * mParams.dGain,
      // That's the derivative of the constant (AKA dMean) I believe...
      v6Jac(4, 0) = 1.0; 
      // And finally, the derivative of the coefficient (AKA dGain)
      v6Jac(5, 0) = mimTemplate(r, c);
