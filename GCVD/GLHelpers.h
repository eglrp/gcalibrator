...
namespace GLXInterface
{
	...
	template <class P> inline void glMultMatrix4x4( const cv::Mat_<P> &m )
	{
		GLdouble glm[16];
	...
	/// @param m the transformation matrix
	template <class P> inline void glMultMatrix3x3( const cv::Mat_<P> &m )
	{
		GLdouble glm[16];
	...
	/// @param m the transformation matrix
	template <class P> inline void glMultMatrix2x2( const cv::Mat_<P> &m )
	{
		GLdouble glm[16];
	...
	inline void glDrawPixelsBGR(const cv::Mat &im)
	{
		...
		glDrawPixels(im.cols,im.rows, GL_BGR, GL_BYTE, im.data);
	...
	inline void glDrawPixelsGRAY(const cv::Mat &im)
	{
		...
		glDrawPixels(im.cols,im.rows, GL_LUMINANCE, GL_BYTE, im.data);
		glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
	...
	{
		::glReadPixels(origin.x, origin.y, im.cols, im.rows, GL_BGR, GL_BYTE, im.data);
	}
	...
	{
		glReadPixels(origin.x, origin.y, im.cols, im.rows, GL_LUMINANCE, GL_BYTE, im.data);
	}
	...
	inline void glTexSubImage2DGRAY( const cv::Mat &img, GLint xoffset = 0, GLint yoffset = 0, GLenum target = GL_TEXTURE_2D, GLint level = 0)
	{
		int alpha = std::min( cvdalignof( img.ptr<void>(0,0) ), 
				      cvdalignof( img.ptr<void>(1,0) ) );
		::glPixelStorei(GL_UNPACK_ALIGNMENT, alpha);
	...
	inline void glTexImage2DBGR( const cv::Mat &img, GLint border = 0, GLenum target = GL_TEXTURE_2D, GLint level = 0)
	{
		int alpha = std::min( cvdalignof( img.ptr<void>(0,0) ), 
				      cvdalignof( img.ptr<void>(1,0) ) );
		::glPixelStorei(GL_UNPACK_ALIGNMENT, alpha);
		::glPixelStorei(GL_UNPACK_ROW_LENGTH, img.step / img.elemSize());
		::glTexImage2D(target, level, GL_BGR, img.cols, img.rows, border, GL_BGR, GL_BYTE, img.data);
		::glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
	}
